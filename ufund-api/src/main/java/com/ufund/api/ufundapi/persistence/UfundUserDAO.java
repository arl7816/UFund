 ...
 */
@Component
public class UfundUserDAO implements UFundDAOUsers {
     ...
     */
    public UfundUserDAO(@Value("${users.file}") String filename,ObjectMapper objectMapper) throws IOException {
...
/*The following method is purely used in createUser() to check whether a user already exsists*/
    private User[] getUser(String containsText){ // if containsText == null, no filter
        ArrayList<User> userArrayList = new ArrayList<>();

        for (User user : users.values()) {
            if (containsText == null || user.getUsername().equals(containsText)) {
                userArrayList.add(user);
            }
        }
    ...
    public User createUser(User user) throws IOException {
        synchronized(users) {
            ...
            User[] userArray = getUser(user.getUsername());
            if (userArray.length>=1){
                return null;
            } else {
            User newUser = new User(nextId(), user.getUsername(), user.getFundingBasket(), user.getPassword());
    ...
    /**
     * TODO: this still needs to be firgued out
     */
